...
namespace ConsoleTNKDxf.Dgts
{
    public class PecaDgt
    {
        ...
        public PecaDgt(TSM.Part pecaChild)
        {
            ...
            _height = doubleProperties.ContainsKey("HEIGHT") ? double.Parse(doubleProperties["HEIGHT"].ToString()) : 0.0;
            _length = doubleProperties.ContainsKey("LENGTH") ? double.Parse(doubleProperties["LENGTH"].ToString()) : 0.0;
            _width = doubleProperties.ContainsKey("WIDTH") ? double.Parse(doubleProperties["WIDTH"].ToString()) : 0.0;
            _profileDiameter = doubleProperties.ContainsKey("PROFILE.DIAMETER") ? double.Parse(doubleProperties["PROFILE.DIAMETER"].ToString()) : 0.0;
            _profilePlateThickness = doubleProperties.ContainsKey("PROFILE.PLATE_THICKNESS") ? double.Parse(doubleProperties["PROFILE.PLATE_THICKNESS"].ToString()) : 0.0;
            _profileWeightPerUnitLength = doubleProperties.ContainsKey("PROFILE.WEIGHT_PER_UNIT_LENGTH") ? double.Parse(doubleProperties["PROFILE.WEIGHT_PER_UNIT_LENGTH"].ToString()) : 0.0;
            _weightNet = doubleProperties.ContainsKey("WEIGHT_NET") ? double.Parse(doubleProperties["WEIGHT_NET"].ToString()) : 0.0;
            _weightGross = doubleProperties.ContainsKey("WEIGHT_GROSS") ? double.Parse(doubleProperties["WEIGHT_GROSS"].ToString()) : 0.0;
            _weightM = doubleProperties.ContainsKey("WEIGHT_M") ? double.Parse(doubleProperties["WEIGHT_M"].ToString()) : 0.0;
